...
## 1. Use Existing LinkedinCRM Database

1. Navigate to your existing LinkedinCRM Supabase project
2. We'll add the Personal CRM tables to this existing database
3. Get your project URL and API keys from the project settings

**Note**: All Personal CRM tables are prefixed with `personal_` to avoid conflicts with existing LinkedinCRM tables.

## 2. Database Schema Setup

Run these SQL commands in the Supabase SQL Editor to add Personal CRM tables:
...
CREATE TABLE personal_contacts (
  ...
  nickname VARCHAR(30),
  birthday DATE, -- Store as MM-DD format
...
CREATE TABLE personal_contact_info (
  ...
  -- Ensure only one primary contact info per type per contact
  UNIQUE(contact_id, type, is_primary) DEFERRABLE INITIALLY DEFERRED
);
...
Add these to your `.env.local` file using your LinkedinCRM project credentials:

```
NEXT_PUBLIC_SUPABASE_URL=your_linkedin_crm_supabase_url
NEXT_PUBLIC_SUPABASE_ANON_KEY=your_linkedin_crm_supabase_anon_key
```

## 4. Row Level Security (RLS) - Single User Setup

Since this is a single-user system, we'll keep RLS simple:
...
Test the connection by running the Next.js app and checking that Supabase client initializes without errors.
