  ...
  private static getTools(): Anthropic.Tool[] {
    return [
      ...
      },
      {
        ...
        description:
          'Get all currently open trading positions (LEAPS). Use when William asks "what positions do I have", "show my trades", "what\'s open", etc.',
        input_schema: {
          type: 'object',
          properties: {},
          required: [],
      ...
      },
      {
        name: 'close_trade',
        description:
          'Close an existing open trade by specifying closing premium. Use when William says he sold/closed a position with the closing price.',
        input_schema: {
          type: 'object',
          properties: {
            trade_id: {
              type: 'string',
              description: 'ID of the trade to close (get from get_open_positions)',
            },
            ticker_symbol: {
              type: 'string',
              description: 'Ticker symbol to identify the trade if trade_id not provided',
            },
            strike_price: {
              type: 'number',
              description: 'Strike price to identify the trade if trade_id not provided',
            },
            closing_premium: {
              type: 'number',
              description: 'Closing premium per contract in dollars',
            },
          },
          required: ['closing_premium'],
        },
      },
      {
        name: 'expire_trades',
        description:
          'Mark trades as expired (for options that expired worthless or were exercised). Use when William mentions trades expiring.',
        input_schema: {
          type: 'object',
          properties: {
            ticker_symbol: {
              type: 'string',
              description: 'Optional: only expire trades for this ticker',
            },
            expiration_date: {
              type: 'string',
              description: 'Optional: only expire trades expiring on this date (YYYY-MM-DD)',
            },
          },
          required: [],
        },
      },
      {
        ...
        description:
          'Get P&L summary and performance statistics. Use when William asks about his trading performance, P&L, win rate, etc.',
        input_schema: {
          type: 'object',
          properties: {},
          required: [],
      ...
      },
      {
        ...
        description:
          'Get positions expiring within a specified number of days. Use when William asks about upcoming expirations.',
        input_schema: {
          type: 'object',
          properties: {
            days_ahead: {
      ...
      },
      {
        name: 'web_search',
        description:
          'Search the web for current information. Use this when you need up-to-date information about news, weather, stock prices, events, or anything that requires current data beyond your knowledge cutoff. Examples: "Barcelona weather tomorrow", "SPY stock price today", "latest AI news".',
        input_schema: {
          type: 'object',
          properties: {
            query: {
      ...
      },
      {
        name: 'web_fetch',
        description:
          'Fetch and summarize content from a specific URL. Use this when the user provides a URL or when you need to read a specific webpage.',
        input_schema: {
          type: 'object',
          properties: {
            url: {
            ...
            question: {
              type: 'string',
              description:
                'Optional: What specific information to extract (e.g., "summarize this article", "what\'s the main point?")',
            },
          },
          required: ['url'],
        },
    ...
    try {
      switch (toolName) {
        ...
        case 'get_open_positions': {
          const trades = await TradingService.getAllTrades(userId)
          ...
          const positions = openTrades.map(t => {
            ...
            const premium = t.premium_per_contract * t.number_of_contracts
            return {
              id: t.id,
              ...
              display: `${t.ticker_symbol} $${t.strike_price} ${t.option_type} (${t.number_of_contracts}x) exp ${t.expiration_date} (${daysToExp}d) - Premium: $${premium.toFixed(2)}`,
            }
          })

          return {
            success: true,
            message: `${openTrades.length} open position${openTrades.length === 1 ? '' : 's'}`,
            positions,
          }
        }
        ...
        case 'close_trade': {
          ...
          let tradeToClose
          if (input.trade_id) {
            tradeToClose = await TradingService.getTradeById(input.trade_id)
          } else if (input.ticker_symbol && input.strike_price) {
            const trades = await TradingService.getAllTrades(userId)
            tradeToClose = trades.find(
              t =>
                t.status === 'OPEN' &&
                t.ticker_symbol === input.ticker_symbol.toUpperCase() &&
                t.strike_price === input.strike_price
            )
          }
          ...
          if (!tradeToClose) {
            return {
              success: false,
              message: 'Could not find open trade to close. Try specifying ticker and strike price.',
            }
          }

          // Close the trade
          const closedTrade = await TradingService.closeTrade(tradeToClose.id, input.closing_premium)

          const pl = closedTrade.realized_pl || 0
          const plSign = pl >= 0 ? '+' : ''

          return {
            success: true,
            message: `Closed ${closedTrade.ticker_symbol} $${closedTrade.strike_price} ${closedTrade.option_type} @ $${input.closing_premium}. P&L: ${plSign}$${pl.toFixed(2)}`,
        ...
        case 'expire_trades': {
          const trades = await TradingService.getAllTrades(userId)
          let tradesToExpire = trades.filter(t => t.status === 'OPEN')

          // Filter by ticker if provided
          if (input.ticker_symbol) {
            tradesToExpire = tradesToExpire.filter(t => t.ticker_symbol === input.ticker_symbol.toUpperCase())
          }

          // Filter by expiration date if provided
          if (input.expiration_date) {
            const expDate = this.parseRelativeDate(input.expiration_date)
            tradesToExpire = tradesToExpire.filter(t => t.expiration_date === expDate)
          } else {
            // Default: only expire trades expiring today or earlier
            const today = new Date().toISOString().split('T')[0]
            tradesToExpire = tradesToExpire.filter(t => t.expiration_date <= today)
          }
          ...
          // Expire all matching trades
          const expiredTrades = await Promise.all(
            tradesToExpire.map(t => TradingService.expireTrade(t.id))
          )
          ...
          return {
            success: true,
            message: `Expired ${expiredTrades.length} trade${expiredTrades.length === 1 ? '' : 's'}. Total P&L: ${plSign}$${totalPL.toFixed(2)}`,
            expired_count: expiredTrades.length,
            total_pl: totalPL,
          }
        }

        case 'get_trading_summary': {
          const summary = await TradingService.getPLSummary(userId)
          ...
          return {
            ...
            summary: {
              ...
              avg_win: summary.avgWin,
              avg_loss: summary.avgLoss,
            },
          }
        }

        case 'get_expiring_positions': {
          const daysAhead = input.days_ahead || 7
          const trades = await TradingService.getAllTrades(userId)
          const openTrades = trades.filter(t => t.status === 'OPEN')
          ...
          const expiringTrades = openTrades.filter(t => {
          ...
          if (expiringTrades.length === 0) {
            return {
              success: true,
              message: `No positions expiring in the next ${daysAhead} days`,
              positions: [],
            }
          }

          const positions = expiringTrades.map(t => {
            const daysToExp = Math.ceil(
              (new Date(t.expiration_date).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24)
            )
            return {
              ...
              expiration: t.expiration_date,
              days_to_exp: daysToExp,
              display: `${t.ticker_symbol} $${t.strike_price} ${t.option_type} exp ${t.expiration_date} (${daysToExp}d)`,
            }
          })

          return {
            success: true,
            message: `${expiringTrades.length} position${expiringTrades.length === 1 ? '' : 's'} expiring in next ${daysAhead} days`,
            positions,
          }
        }
        ...
        case 'web_fetch': {
          // Call the web fetch API endpoint
          const baseUrl = process.env.VERCEL_URL
            ? `https://${process.env.VERCEL_URL}`
            : 'http://localhost:3000'

          const response = await fetch(`${baseUrl}/api/web-fetch`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              url: input.url,
              question: input.question || 'Summarize this webpage',
            }),
          })
